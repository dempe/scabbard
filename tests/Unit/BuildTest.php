...
class BuildTest extends TestCase
{
  ...
  public function test_build_site_excludes_drafts_by_default(): void
  {
    $tempContentDir = base_path('tests/tmp_content');
    $tempOutputDir = base_path('tests/tmp_output');
    $draftsDir = $tempContentDir . '/drafts';

    File::deleteDirectory($tempContentDir);
    File::deleteDirectory($tempOutputDir);

    File::ensureDirectoryExists($tempContentDir);
    File::ensureDirectoryExists($draftsDir);
    File::put("{$tempContentDir}/published.txt", 'published');
    File::put("{$draftsDir}/draft.txt", 'draft');

    Config::set('scabbard.copy_dirs', [$tempContentDir]);
    Config::set('scabbard.routes', []);
    Config::set('scabbard.output_path', $tempOutputDir);
    Config::set('scabbard.drafts_path', $draftsDir);

    Artisan::call('scabbard:build');

    $this->assertTrue(File::exists("{$tempOutputDir}/published.txt"));
    $this->assertFalse(File::exists("{$tempOutputDir}/drafts/draft.txt"));

    File::deleteDirectory($tempContentDir);
    File::deleteDirectory($tempOutputDir);
  }

  public function test_build_site_includes_drafts_when_option_enabled(): void
  {
    $tempContentDir = base_path('tests/tmp_content');
    $tempOutputDir = base_path('tests/tmp_output');
    $draftsDir = $tempContentDir . '/drafts';
    ...
    $this->assertTrue(File::exists("{$tempOutputDir}/published.txt"));
    $this->assertTrue(File::exists("{$tempOutputDir}/drafts/draft.txt"));
